...
class GameManager: ObservableObject {
    ...
    init(puzzleMode: PuzzleMode, puzzle: Puzzle? = nil) {
        ...
        if puzzle != nil { self.puzzle = puzzle! }
        else { self.puzzle = Puzzle(difficulty: puzzleMode) }
    ...
    func switchMode() {
        if self.entryMode == .marker { self.entryMode = .pencil }
        else if self.entryMode == .pencil { self.entryMode = .marker }
    }
